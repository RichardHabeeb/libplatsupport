...
static struct omap_gpio_prv {
    //mux_sys_t* mux;
    volatile omap_gpio_bank* bank[GPIO_NBANKS];
} _gpio;
 ...
 */
inline volatile static uint32_t omap_gpio_read_reg(omap_gpio_bank* bank, uint16_t reg)
{
    return (*((volatile uint32_t *)(bank->base + reg)));
}

inline volatile static uint32_t omap_gpio_write_reg(omap_gpio_bank* bank, uint16_t reg, uint32_t v)
{
    (*((volatile uint32_t *)(bank->base + reg))) = v;
}

static inline void omap_gpio_rmw(omap_gpio_bank* bank, uint16_t reg, uint32_t mask, bool set)
{
    int l = omap_gpio_read_reg(bank->base, reg);
    ...
    omap_gpio_write_reg(bank->base, reg, l);
}

volatile static struct omap_gpio_bank* omap_gpio_get_bank(gpio_t* gpio) {
...
static void omap_gpio_set_direction(gpio_t* gpio, enum gpio_dir dir)
{
    ...
    omap_gpio_rmw(bank, bank->regs->direction, BIT(GPIOID_PORT(gpio->id)), dir == GPIO_DIR_OUT);
}
...
static void omap_gpio_write(gpio_t* gpio, const char* data, int len)
{
    int i;
    for (i = 0; i < len && gpio; i++) {
        omap_gpio_bank* bank = omap_gpio_get_bank(gpio);
...
{
    if (bank0 != NULL) {
        _gpio.bank[GPIO_BANK0]->base = bank0;
    }
    if (bank1 != NULL) {
        _gpio.bank[GPIO_BANK1]->base = bank1;
    }
    if (bank2 != NULL) {
        _gpio.bank[GPIO_BANK2]->base = bank2;
    }
    if (bank3 != NULL) {
        _gpio.bank[GPIO_BANK3]->base = bank3;
    }
    return omap4_gpio_init_common(mux, gpio_sys);
}
